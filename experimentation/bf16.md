# fp32 vs bf16 mixed precision ablation
...
All hyperparameters are identical between runs except for automatic mixed precision. Both runs use the same tokenizer (BPE, 500 merges), same dataset (`shakespeare.txt`), same device (MPS), and were run with no other applications open to keep ms/iter comparable.
...
Peformance in terms of loss/bpb is extremely similar, which makes sense since the model is the same besides the mixed precision. The bf16 run is slightly slower on average, but enough that I need a second run to confirm if this is just randomness/noise versus a real effect of bf16.
...
Bf16 is slower than fp32 on MPS, which is the opposite of what I expected. This may have something to do with the fact that I'm running on MPS and not CUDA so I don't have tensor cores. Additionally, it's interesting that bf16 ms/iter almost oscillates, increasing and decreasing across iterations but with changing ranges.

This experiment is inconclusive because without CUDA I'm not able to accurately test bf16 vs fp32. But, it was a good thought exercise to understand the difference between the two. Next steps would be to try running this with CUDA or to dig deeper into the "oscillation."
